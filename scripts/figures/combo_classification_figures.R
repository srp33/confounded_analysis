...
# Function to create title from dataset name
create_title <- function(dataset_name) {
...
order <- c("unadjusted", "min_mean", "fastMNN", "npn", "npn_global", "gmm", "gmm_scale_separate", "gmm_npn", "gmm_npn_unit_std_global")

score_functions <- c("roc_auc_score", "mutual_info_score", "accuracy_score", "matthews_corrcoef")
...
# Get unique datasets from the data (after loading)
unique_datasets <- unique(batchall$dataset)

# Debugging checks
check_column(batchall, "dataset", unique_datasets)
check_column(trueall, "dataset", unique_datasets)
...
for (dataset_name in unique_datasets) {
  title <- create_title(dataset_name)
  batch_col_name <- "meta_source"
  true_col_name <- "meta_er_status"
  ...
  for(score_function in score_functions) {
    ...
    ggsave(file.path(FIG_DIR, filename_for_plot), width = 11, height = 8.5, units = 'in', create.dir=TRUE)
  }
}
