# generate_relative_plot.R
# Generate relative performance plot showing differences from top adjuster

#' Generate relative performance plot
#' @param mxe_data Data frame with MCC values
#' @param top_adjuster Name of top performing adjuster
#' @param top_adjuster_label Formatted label for top adjuster
#' @param output_file Path to save plot
#' @param width Plot width in inches
#' @param height Plot height in inches
#' @param dpi Plot resolution
generate_relative_plot <- function(mxe_data, top_adjuster, top_adjuster_label, 
                                   output_file, width = 20, height = 16, dpi = 300) {
  
  cat("\nCreating relative performance plot (classifier-specific best adjuster)...\n")
  
  # Get classifiers with data
  classifiers_with_data <- unique(mxe_data$classifier_label[!is.na(mxe_data$classifier_label)])
  
  # Collect all significance results for saving
  all_sig_results <- data.frame()
  
  # Create individual plots for each classifier
  relative_plot_list <- list()
  
  for (classifier in classifiers_with_data) {
    # Get data for this classifier
    classifier_data <- mxe_data[mxe_data$classifier_label == classifier, ]
    
    if (nrow(classifier_data) == 0) {
      cat(sprintf("Skipping %s - no data\n", classifier))
      next
    }
    
    # Get classifier-specific ordering and best adjuster
    classifier_adjuster_order <- get_classifier_ordering(mxe_data, classifier)
    classifier_specific_labels <- create_adjuster_labels(classifier_adjuster_order)
    classifier_best_adjuster <- classifier_adjuster_order$adjuster[1]
    classifier_best_label <- classifier_specific_labels[1]
    
    cat(sprintf("Classifier %s: best adjuster is %s\n", classifier, classifier_best_label))
    
    # Calculate relative values for this classifier (relative to its best adjuster)
    # Negative values = worse than best (below zero line)
    raw_data <- classifier_data %>%
      select(adjuster, classifier, classifier_label, n_datasets, test_study, value) %>%
      mutate(condition_id = paste(classifier, n_datasets, test_study, sep = "_")) %>%
      group_by(condition_id) %>%
      mutate(best_value = value[adjuster == classifier_best_adjuster]) %>%
      ungroup() %>%
      mutate(relative_value = value - best_value) %>%  # Negative = worse than best
      select(-best_value, -condition_id) %>%
      mutate(dataset_label = factor(paste(n_datasets, "studies"),
                                    levels = c("3 studies", "4 studies", "5 studies", "6 studies")))
    
    # Apply ordering
    raw_data$adjuster_label <- factor(raw_data$adjuster,
                                      levels = classifier_adjuster_order$adjuster,
                                      labels = classifier_specific_labels)
    
    # Statistical testing: compare each adjuster to best, separately for each n_datasets
    # This gives us p-values per adjuster per n_datasets
    pval_data <- data.frame()
    
    for (n_studies in c(3, 4, 5, 6)) {
      for (adj_label in classifier_specific_labels[-1]) {  # Skip first (best)
        adj_data <- raw_data[raw_data$adjuster_label == adj_label & 
                            raw_data$dataset_label == paste(n_studies, "studies"), ]
        
        if (nrow(adj_data) > 1) {
          # One-sample t-test: is relative_value significantly < 0?
          # (negative means this adjuster is worse than best)
          t_result <- t.test(adj_data$relative_value, mu = 0, alternative = "less")
          
          pval_data <- rbind(pval_data, data.frame(
            adjuster_label = adj_label,
            dataset_label = paste(n_studies, "studies"),
            p_value = t_result$p.value,
            stringsAsFactors = FALSE
          ))
        }
      }
    }
    
    # Apply FDR correction across all comparisons within this classifier
    if (nrow(pval_data) > 0) {
      cat(sprintf("  Applying FDR correction across %d comparisons for %s\n", 
                  nrow(pval_data), classifier))
      pval_data$p_adj <- p.adjust(pval_data$p_value, method = "fdr")
      
      # Add significance labels (blank for non-significant)
      pval_data$sig_label <- sapply(pval_data$p_adj, function(p) {
        if (p < 0.05) return("*")
        return("")  # Blank for non-significant
      })
      
      # Add classifier and best adjuster info for saving
      pval_data$classifier <- classifier
      pval_data$best_adjuster <- classifier_best_label
      
      # Collect for saving
      all_sig_results <- rbind(all_sig_results, pval_data)
    }
    
    # Calculate means
    mean_data <- raw_data %>%
      group_by(adjuster_label, dataset_label) %>%
      summarise(mean_value = mean(relative_value, na.rm = TRUE), .groups = "drop")
    
    # Calculate y-axis limits (add more space for annotations)
    local_y_max <- max(raw_data$relative_value, na.rm = TRUE)
    local_y_min <- min(raw_data$relative_value, na.rm = TRUE)
    local_y_range <- local_y_max - local_y_min
    y_limits <- c(local_y_min - 0.05 * local_y_range, 
                  local_y_max + 0.15 * local_y_range)  # More space for annotations
    
    # Create plot
    p <- ggplot(raw_data, aes(x = adjuster_label, y = relative_value)) +
      geom_hline(yintercept = 0, linetype = "solid", color = "black", linewidth = 0.6, alpha = 0.7) +
      geom_segment(data = mean_data, 
                  aes(x = as.numeric(adjuster_label) - 0.3, 
                      xend = as.numeric(adjuster_label) + 0.3,
                      y = mean_value, yend = mean_value),
                  color = "gray40", linewidth = 0.8) +
      geom_point(aes(color = test_study, shape = test_study), size = 3, alpha = 0.5) +
      facet_wrap(~ dataset_label, scales = "fixed", ncol = 4) +
      scale_x_discrete(drop = FALSE) +
      scale_y_continuous(limits = y_limits, expand = expansion(mult = c(0, 0))) +
      scale_color_brewer(palette = "Set2", name = "Test Study") +
      scale_shape_manual(values = c(15, 16, 17, 25, 18, 19), name = "Test Study") +
      theme_bw() +
      theme(
        axis.title.x = element_blank(),
        axis.text.x = element_text(angle = 30, hjust = 1, size = 8),
        axis.title.y = element_text(size = 10),
        legend.title = element_text(size = 9, face = "bold"),
        legend.position = "right",
        panel.grid.major.y = element_line(color = "grey90", size = 0.5),
        panel.grid.major.x = element_blank(),
        panel.grid.minor = element_blank(),
        strip.text = element_text(size = 9),
        plot.title = element_text(size = 12, hjust = 0.5)
      ) +
      labs(
        y = sprintf("Relative MCC\n(vs. %s)", classifier_best_label),
        title = classifier
      )
    
    # Add p-value annotations if we have them
    if (nrow(pval_data) > 0) {
      annotation_y <- local_y_max + 0.08 * local_y_range
      pval_data$y_pos <- annotation_y
      
      # Ensure dataset_label is a factor with same levels
      pval_data$dataset_label <- factor(pval_data$dataset_label,
                                       levels = c("3 studies", "4 studies", "5 studies", "6 studies"))
      
      p <- p + geom_text(data = pval_data,
                        aes(x = adjuster_label, y = y_pos, label = sig_label),
                        inherit.aes = FALSE,
                        size = 4, fontface = "bold", vjust = 0)
    }
    
    relative_plot_list[[classifier]] <- p
  }
  
  # Arrange and save
  cat("Arranging relative plots in grid\n")
  relative_final_plot <- grid.arrange(grobs = unname(relative_plot_list), ncol = 2)
  
  cat("Saving relative performance plot to:", output_file, "\n")
  ggsave(filename = output_file, plot = relative_final_plot, 
         width = width, height = height, dpi = dpi, units = "in")
  
  cat("Relative performance plot saved successfully!\n")
  
  # Save significance results
  if (nrow(all_sig_results) > 0) {
    sig_output <- sub("\\.png$", "_significance.csv", output_file)
    write.csv(all_sig_results, sig_output, row.names = FALSE)
    cat("Significance results saved to:", sig_output, "\n")
  }
}
