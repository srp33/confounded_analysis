...
for(ID in 1:iterations){
  ...
  for(l_type in learner_types){
    ...
    if(inherits(pred_base_res, "try-error")){ID <- ID - 1; break; next}
    ...
    pred_batch_res <- try(trainPipe(train_set=train_expr_batch_whole_norm, train_label=curr_y_train, 
                                    test_set=test_expr_norm, 
                                    lfit=learner_fit, use_ref_combat=use_ref_combat))
    if(inherits(pred_batch_res, "try-error")){ID <- ID - 1; break; next}
    ...
    if(inherits(train_expr_combat, "try-error")){ID <- ID - 1; break; next}
    if(norm_data){
    ...
    if(inherits(pred_sgbatch_res, "try-error")){ID <- ID - 1; break; next}
    ...
    if(inherits(cs_zmat, "try-error")){ID <- ID - 1; break; next}
    ...
    reg_ssl_res <- try(Reg_SSL_pred(study_lst=train_lst, label_lst=y_sgbatch_train, 
                                    lfit=learner_fit, use_ref_combat=use_ref_combat))
    if(inherits(reg_ssl_res, "try-error")){ID <- ID - 1; break; next}
