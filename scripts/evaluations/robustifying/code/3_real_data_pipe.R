# 5-Study Real Data Analysis for Robustifying Evaluation
...
suppressMessages(suppressWarnings({
  rm(list=ls())
  if(!dir.exists("~/confounded_analysis/scripts/evaluations/robustifying/results_real_3studies")){
    dir.create("~/confounded_analysis/scripts/evaluations/robustifying/results_real_3studies")
  }
...
# Make sure only 3 studies are included
drop_studies <- c("GSE37250_M", "GSE39941_M", "Africa")
dat_lst <- dat_lst[!(names(dat_lst) %in% drop_studies)]
label_lst <- label_lst[!(names(label_lst) %in% drop_studies)]
study_names <- names(dat_lst)
cat("Running 3-study analysis with studies:", paste(study_names, collapse=", "), "\n")
...
write_performance_results <- function(perf_data, test_name, perf_measure, iteration) {
  ...
  filepath <- sprintf('/scripts/evaluations/robustifying/results_real_3studies/test%s_%s.csv',
...
for(s in study_names) {
  ...
  save(navg_weights, 
       ...
       file=sprintf('/scripts/evaluations/robustifying/results_real_3studies/test%s_weights.RData', test_name))
...
cat("3-study analysis completed successfully\n")