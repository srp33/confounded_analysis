...
suppressMessages(suppressWarnings({
  rm(list=ls())
  if(!dir.exists("~/confounded_analysis/scripts/evaluations/robustifying/results_real_5studies")){
    dir.create("~/confounded_analysis/scripts/evaluations/robustifying/results_real_5studies")
  }
...
# Make sure only 5 studies are included
dat_lst <- dat_lst[names(dat_lst) != "GSE39941_M"]
labels_lst <- label_lst[names(label_lst) != "GSE39941_M"]
study_names <- names(dat_lst)
cat("Running 5-study analysis with studies:", paste(study_names, collapse=", "), "\n")
...
write_performance_results <- function(perf_data, test_name, perf_measure, iteration) {
  ...
  filepath <- sprintf('/scripts/evaluations/robustifying/results_real_5studies/test%s_%s.csv',
...
for(s in study_names) {
  ...
  save(navg_weights, 
       ...
       file=sprintf('/scripts/evaluations/robustifying/results_real_5studies/test%s_weights.RData', test_name))
...
cat("5-study analysis completed successfully\n")