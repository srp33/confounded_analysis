...
# BatchQC Analysis Script for Batch Effect Visualization
# Generates comprehensive batch effect reports with multiple visualizations
...
if (length(args) < 2) {
  stop("Usage: Rscript batchqc_analysis.R <input_file> <output_dir>")
}
...
cat("BatchQC analysis with:", input_file, "\n")
...
# Take up to 100 samples from each batch for analysis
...
# Take first 500 genes for analysis
test_genes <- head(expr_cols, 500)

# Subset data
data_subset <- data[selected_indices, c(test_genes, meta_cols)]
cat("Analysis subset dimensions:", nrow(data_subset), "samples x", ncol(data_subset), "columns\n")
...
cat("\n✓ BatchQC analysis completed successfully!\n")
