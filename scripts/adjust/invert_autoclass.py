...
def AutoClassImpute(data, cellwise_norm=True, log1p=True, num_cluster=9,
                    ...
                    batchlabel=[], reg=0.000, batch_size=32, epochs=300,
    ...
    batchlabel: 'tuple' or 'list'. Default: [].
...
class AutoClass(object):
    ...
    def set_batchlabel(self, batchlabel):
        n = len(batchlabel)
        assert n == self.ncell, 'length of batch label should equal to the number of cells'
        labelid = pd.factorize(batchlabel)[0]
